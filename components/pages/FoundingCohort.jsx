...
export default function FoundingCohort() {
  ...
  const benefits = t.raw('foundingCohort.benefits')
  const howSteps = t.raw('foundingCohort.howSteps')
  const whyReasons = t.raw('foundingCohort.whyReasons')
  const profileItems = t.raw('foundingCohort.profileItems')
  const selectionItems = t.raw('foundingCohort.selectionItems')
  const toolkitItems = t.raw('foundingCohort.toolkitItems')
  const faqItems = t.raw('foundingCohort.faqItems')
      ...
      <section style={{ background: 'var(--navy)', padding: '80px 60px' }} className="section-pad">
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'start' }} className="grid-2">
          ...
          <FadeIn delay={0.15}>
            ...
            {/* Rows */}
            {t.raw('foundingCohort.mathRows').map(([label, test, next2, onboarding, forLife, standard], i) => (
