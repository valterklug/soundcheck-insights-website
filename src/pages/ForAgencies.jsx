...
export default function ForAgencies() {
  ...
  const chips = t('forAgencies.chips', { returnObjects: true })
  const operatorItems = t('forAgencies.operatorItems', { returnObjects: true })
  const whiteLabelProducts = t('forAgencies.whiteLabelProducts', { returnObjects: true })
  const worksWellItems = t('forAgencies.worksWellItems', { returnObjects: true })
  const notRightItems = t('forAgencies.notRightItems', { returnObjects: true })

  // Process white-label products to add localized links
  const productsWithLinks = whiteLabelProducts.map(p => ({
    ...p,
    link: getLocalizedPath(p.link || '/products', lang)
  }))
    ...
    <PageWrapper>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:80,alignItems:"start"}} className="grid-2">
            ...
            <FadeIn delay={0.1}>
              ...
              <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",padding:"22px 24px",marginTop:20}}>
                ...
                <Link to={getLocalizedPath('/contact', lang)} className="text-link">{t('forAgencies.notSureLink')}</Link>
              </div>
            </FadeIn>
          </div>
        </div>
      ...
      <section className="cta-strip">
        ...
        <div className="cta-actions">
          <Link to={getLocalizedPath('/partner', lang)} className="btn btn-white">{t('forAgencies.ctaButton')}</Link>
        </div>
      </section>
    </PageWrapper>
  )
