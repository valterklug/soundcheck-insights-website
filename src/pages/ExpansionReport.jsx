...
export default function ExpansionReport() {
  ...
  const reportSections = t('expansionReport.sections', { returnObjects: true })
  const cveScale = t('expansionReport.cveScale', { returnObjects: true })
  const principles = t('expansionReport.principles', { returnObjects: true })
  const faqs = t('expansionReport.faqs', { returnObjects: true })
    ...
    <PageWrapper>
      ...
      <section className="page-hero" style={{ borderBottom: '3px solid var(--teal)' }}>
        <div className="page-hero-inner">
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 28 }}>
            <Link to={getLocalizedPath('/contact', lang)} className="btn btn-primary">{t('expansionReport.briefButton')}</Link>
          ...
          <div style={{ display: 'flex', gap: 40, marginTop: 40, flexWrap: 'wrap' }}>
            {t('expansionReport.heroStats', { returnObjects: true }).map(({ num, label, sub }) => (
      ...
      <section style={{ background: 'var(--navy)', padding: '80px 60px' }} className="section-pad">
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <StaggerContainer style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2, marginTop: 32 }} className="offer-grid">
            {t('expansionReport.whoCards', { returnObjects: true }).map((card) => (
          ...
          <FadeIn delay={0.1}>
            <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 2, marginBottom: 32 }} className="cve-grid">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {t('expansionReport.cveDimensions', { returnObjects: true }).map((dim) => (
      ...
      <section style={{ background: 'var(--navy)', padding: '80px 60px', borderTop: '1px solid var(--border)' }} className="section-pad">
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2 }} className="offer-grid">
            {t('expansionReport.paths', { returnObjects: true }).map((path) => (
      ...
      <section style={{ background: 'var(--navy-2)', padding: '80px 60px', borderTop: '1px solid var(--border)' }} className="section-pad">
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }} className="grid-2">
            {t('expansionReport.deliverables', { returnObjects: true }).map((deliv, idx) => (
          ...
          <FadeIn delay={0.1}>
            <div style={{ background: 'rgba(0,196,212,0.03)', border: '1px solid rgba(0,196,212,0.15)', padding: '48px 40px', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 40, flexWrap: 'wrap', marginBottom: 28 }}>
                {t('expansionReport.pricingFeatures', { returnObjects: true }).map((feature) => (
              ...
              <Link to={getLocalizedPath('/contact', lang)} className="btn btn-primary" style={{ fontSize: 14 }}>{t('expansionReport.briefButton')}</Link>
            </div>
          </FadeIn>
        </div>
      </section>
      ...
      <section className="cta-strip">
        ...
        <div className="cta-actions" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          <Link to={getLocalizedPath('/contact', lang)} className="btn btn-white">{t('expansionReport.ctaButton')}</Link>
          <Link to={getLocalizedPath('/products', lang)} style={{ fontFamily: 'IBM Plex Sans, sans-serif', fontSize: 13, color: 'rgba(255,255,255,0.7)', textDecoration: 'none' }}>{t('expansionReport.ctaLink')}</Link>
        </div>
      </section>
    </PageWrapper>
  )
